...
export function LoginPage() {
  ...
  const auth = useAuth();
  const [form, setForm] = useState({ email: 'admin@jobscheduler.local', password: 'Admin@12345' });
      ...
      <div className={`relative w-full max-w-6xl overflow-hidden rounded-[32px] border lg:grid lg:grid-cols-[1.05fr_0.95fr] ${panelClasses}`}>
        <div className={`relative flex flex-col justify-between overflow-hidden p-8 lg:p-10 ${heroClasses}`}>
          ...
          <div className="relative">
            ...
            <h1 className={`mt-6 text-3xl font-semibold leading-tight sm:text-4xl ${isDark ? 'text-slate-50' : 'text-ink'}`}>Coordinate queues, workers, and retries across your background pipelines.</h1>
            <p className={`mt-3 max-w-xl text-base leading-7 ${isDark ? 'text-slate-300' : 'text-steel'}`}>Operate your scheduling platform with visibility into execution health, dead-letter recovery, and worker availability.</p>
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <div className={`rounded-2xl border px-4 py-3 text-sm shadow-card ${cardClasses}`}>
                <div className={`flex items-center gap-2 font-semibold ${isDark ? 'text-slate-100' : 'text-ink'}`}><Sparkles className="h-4 w-4 text-accent" />Live insights</div>
              </div>
            </div>
          </div>
          <div className={`relative mt-8 rounded-[24px] border p-5 text-sm shadow-card ${cardClasses}`}>
            <div className={`font-semibold ${isDark ? 'text-slate-100' : 'text-ink'}`}>What you can manage</div>
            <ul className="mt-3 space-y-2">
              <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-accent" />Reliable dispatch to distributed workers</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-accent" />Real-time queue and worker health monitoring</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-accent" />Retry and dead-letter recovery workflows</li>
            </ul>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <div className={`rounded-2xl border p-3 ${statClasses}`}>
                <div className="text-xs uppercase tracking-[0.24em] text-slate-400">Queues</div>
                <div className={`mt-1 text-lg font-semibold ${isDark ? 'text-slate-100' : 'text-ink'}`}>24/7</div>
              </div>
              <div className={`rounded-2xl border p-3 ${statClasses}`}>
                <div className="text-xs uppercase tracking-[0.24em] text-slate-400">Workers</div>
                <div className={`mt-1 text-lg font-semibold ${isDark ? 'text-slate-100' : 'text-ink'}`}>Live</div>
              </div>
              <div className={`rounded-2xl border p-3 ${statClasses}`}>
                <div className="text-xs uppercase tracking-[0.24em] text-slate-400">Recovery</div>
                <div className={`mt-1 text-lg font-semibold ${isDark ? 'text-slate-100' : 'text-ink'}`}>Automated</div>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <div className={`rounded-[24px] border p-6 shadow-card ${isDark ? 'border-slate-800 bg-slate-950/80' : 'border-line bg-slate-50/80'}`}>
            ...
            <h2 className={`mt-4 text-2xl font-semibold ${isDark ? 'text-slate-50' : 'text-ink'}`}>Sign in</h2>
            <p className={`mt-1 text-sm leading-6 ${isDark ? 'text-slate-400' : 'text-steel'}`}>Access the job scheduler control plane for queues, workers, and execution health.</p>
            ...
            <div className={`mt-4 text-center text-sm ${isDark ? 'text-slate-400' : 'text-steel'}`}>
              New user? <Link className="font-medium text-accent" to="/register">Create an account</Link>
            </div>
          </div>
        </form>
      </div>
